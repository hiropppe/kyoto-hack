...
# KNP auto.dat
auto_dat = set()
for l in codecs.open('auto.dat', 'r', 'utf-8'):
  cols = re.split(r'[\+ ]', l[:-1])[:-1]
  w = ''.join([col.split('/')[0] for col in cols])
  auto_dat.add(w)

hyponymy_db = cdb.init('hyponymy.cdb'.encode('utf8'))
#single_mrph_hyponymy = ('駅'.encode('utf8'),)
...
def main(args):
  if args['download']:
    for f in dump_files:
      ...
      url.urlretrieve(os.path.join(dump_base_url, f), f)
      print('Download completed')

  redirect_dict = get_redirect_dict()
  try:
    for title in extract_titles_from_abstract_xml():
      if not title in redirect_dict: 
        process(title, redirect=None)
    
    for title, redirect in redirect_dict.iteritems():
      process(title, redirect)
  finally:
    write(sorted(single_mrph_words), 'single_morph.txt')
    write(sorted(multiple_mrph_words),  'multiple_morph.txt')
...
def process(title, redirect=None):
  ...
  try:
    ...
    #elif ztitle in auto_dat:
    #  debug_container['G_auto'].append(title)
...
def get_redirect_dict():
  id_redirect = extract_id2title_from_sql('jawiki-latest-redirect.sql.gz')
  id_title = extract_id2title_from_sql('jawiki-latest-page.sql.gz')
